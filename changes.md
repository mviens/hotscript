...
> * <kbd>Alt</kbd> + <kbd>Win</kbd> + <kbd>F1</kbd>
...
> Added HotString `@+-` to generate the plus-minus sign `±` character.
...
> Changed `selfReload()` to be more selective about silently reloading. This should only occur when the parameter is actually `false` instead of just being present.
...
> Modified `resizeTo()` for Windows 10. This affects all window movement HotKeys. Windows 10 puts transparent borders around non-maximized windows along the left, right and bottom of a window. When these windows line up next to each other, "gaps" appear which show whatever is underneath the windows. I found this quite annoying, so the math was changed for Windows 10 to accommodate these gaps. The coordinates are adjusted as follows:  X +6, Y +1, Width +16, Height +9.
...
> Added status (`enabled` or `suspended`) to system tray icon tool-tip.
